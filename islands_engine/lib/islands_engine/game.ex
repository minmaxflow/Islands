defmodule IslandsEngine.Game do
  ...
  def init(name) do
    IO.inspect("GenServer game init with name: #{name}")
  ...
  def handle_call({:guess_coordinate, player_key, row, col}, _from, state_data) do
    oppoent_key = oppoent(player_key)
    oppoent_board = player_board(state_data, oppoent_key)

    with {:ok, rules} <- Rules.check(state_data.rules, {:guess_coordinate, player_key}),
         {:ok, coordinate} <- Coordinate.new(row, col),
         {hit_or_miss, forested_island, win_status, oppoent_board} <-
           Board.guess(oppoent_board, coordinate),
         {:ok, rules} <- Rules.check(rules, {:win_check, win_status}) do
      state_data
      |> update_board(oppoent_key, oppoent_board)
  ...
  defp update_play2_name(state_data, name) do
  ...
  defp oppoent(:player1), do: :player2
  defp oppoent(:player2), do: :player1
...
#   start_link/2 
# {:ok, sup} = Supervisor.start_link([Game], strategy: :simple_one_for_one)

# 方法2 
# 创建一个supervisor module:  game_supervisor.ex
